...
Eigen::Vector2d Bug1::get_to_boundary(amp::Problem2D problem, int idx, Eigen::Vector2d wpt, Eigen::Vector2d dir, float dx, float e_o) {
    ...
    if (inside)
    {
        ...
        Eigen::Vector2d t_dir = R * dir; // dir
...
amp::Path2D Bug1::move_along_boundary(amp::Path2D path, amp::Problem2D problem, Eigen::Vector2d q, Eigen::Vector2d dir, int idx, std::vector<std::vector<int>> adj, float dx, float e_o) {
    ...
    Eigen::Vector2d q_c, t_obs, q_b0, q_b1, n_obs;

    // First turn is left
    Eigen::Vector2d dv(-dir[1], dir[0]); // get direction vector
    q = q + 0.1*dx*dv; // get a canidate waypoint
    q = get_to_boundary(problem, idx, q, dv, dx, e_o); // do one correction 

    // // Get the obstacle normal vector
    // q_b1 = get_to_boundary(problem, idx, q, dv, dx, e_o);
    // q_b0 = get_to_boundary(problem, idx, q - 0.01*q, dv, dx, e_o);
    // n_obs = (q_b1 - q_b0) / (q_b1 - q_b0).norm();
    ...
    for (size_t i = 1; i < 1000; i++)
    {
        ...
        // Get the obstacle normal vector
        q_b1 = get_to_boundary(problem, idx, q, t_obs, dx, e_o);
        q_b0 = get_to_boundary(problem, idx, q - 0.0001*q, t_obs, dx, e_o);
        n_obs = (q_b1 - q_b0) / (q_b1 - q_b0).norm();
        ...
        q = get_to_boundary(problem, idx, q, n_obs, dx, e_o);
