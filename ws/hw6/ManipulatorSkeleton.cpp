...
Resources:
- Lecture slides
- Roboscience book for the IK double checking
...
// Try restoring to defalut or overloading
...
amp::ManipulatorState MyManipulator2D::getConfigurationFromIK(const Eigen::Vector2d& end_effector_location) const {
    ...
    if (nLinks() == 2) {
        ...
        double cos_theta_2 = (x*x + y*y - a[0]*a[0] - a[1]*a[1]) / (2.0 * a[0] * a[1]);
        double sin_theta_2 = std::sqrt(std::max(0.0, 1.0 - cos_theta_2*cos_theta_2));

        // Get specific angles
        double theta_2 = std::atan2(sin_theta_2, cos_theta_2);
        double theta_1 = std::atan2(y, x) - std::atan2(a[1]*sin_theta_2, a[0] + a[1]*cos_theta_2);
        ...
        for (size_t i = 0; i < iter; i++)
        {
            ...
            double theta_1 = i * 2 * pi / iter;//atan2(y, x) + dist(gen);
